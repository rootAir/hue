...
(function(factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    (function($, window, document, undefined) {
      ...
      $
          .extend(
              Buttons.prototype,
              {
                ...
                /**
                 * Add an active class to the button to make to look active or get current active state.
                 * 
                 * @param {int|string}
                 *            Button index
                 * @param {boolean}
                 ...
                 * 
                 * @param {int|string}
                 *            Button index for where to insert the button
                 * @param {object}
                 *            Button configuration object, base string name or function
                 * @return {Buttons} Self for chaining
                 */
                ...
                /**
                 * Destroy the instance, cleaning up event handlers and removing DOM elements
                 * 
                 * @return {Buttons} Self for chaining
                 */
                destroy : function() {
                ...
                /**
                 * Tidy up any buttons that have been scheduled for removal. This is required so multiple buttons can be removed without upsetting the
                 * button indexes while removing them.
                 * 
                 * @param {int|string}
                 *            Button index
                 * @return {Buttons} Self for chaining
                ...
                /**
                 * Scheduled a button for removal. This is required so multiple buttons can be removed without upsetting the button indexes while
                 * removing them.
                 * 
                 * @return {Buttons} Self for chaining
                 */
                ...
                text : function(idx, label) {
                  ...
                  var linerTag = typeof idx === 'string' && idx.indexOf('-') !== -1 && buttonLiner && buttonLiner.tag ? buttonLiner.tag
                      : this.c.dom.buttonLiner.tag;
                  var dt = this.s.dt;
                  var text = function(opt) {
                    return typeof opt === 'function' ? opt(dt, button.node, button.conf) : opt;
                  };
                 ...
                 * 
                 * @param {node}
                 *            Button node (_not_ a jQuery object)
                 ...
                 * 
                 * @param {object}
                 *            Resolved button configuration object
                 * @private
                 */
                 ...
                 *            Buttons to create
                 * @param {jQuery}
                 *            Container node into which the created button should be inserted.
                 * @param {int}
                 *            Counter for sub-buttons to be stored in a collection
                 * @private
                 */
                _buildButtons : function(buttons, container, collectionCounter) {
                  ...
                  for (var i = 0, ien = buttons.length; i < ien; i++) {
                    ...
                    var button = this._buildButton(conf, collectionCounter !== undefined ? true : false);
                 ...
                 * 
                 * @param {object}
                 *            config Resolved button configuration
                 * @param {boolean}
                 *            collectionButton `true` if a collection button
                ...
                _buildButton : function(config, collectionButton) {
                  ...
                  var button = $('<' + buttonDom.tag + '/>').addClass(buttonDom.className).attr('tabindex',
                      this.s.dt.settings()[0].iTabIndex).attr('aria-controls', this.s.dt.table().node().id).on(
                      'click.dtb', function(e) {
                        e.preventDefault();
                  ...
                  if (linerDom.tag) {
                    button.append($('<' + linerDom.tag + '/>').html(text(config.text)).addClass(linerDom.className));
                  } else {
                 ...
                 * @param {int|string}
                 *            Button index
                 * @return {object} Button information - object contains `node` and `conf` properties
                 * @private
                 */
                ...
                /**
                 * Handle a key press - determine if any button's key configured matches what was typed and trigger the action if so.
                 * 
                 * @param {string}
                 *            The character pressed
                 * @param {object}
                 *            Key event that triggered this call
                 * @private
                 */
                ...
                /**
                 * Remove a key from the key listener for this instance (to be used when a button is removed)
                 * 
                 * @param {object}
       ...
       * 
       * @param {boolean}
       *            Flag to indicate if the background should be shown or hidden
       * @param {string}
      ...
      /**
       * Instance selector - select Buttons instances based on an instance selector value from the buttons assigned to a DataTable. This is only
       * useful if multiple instances are attached to a DataTable.
       * 
       * @param {string|int|array}
       *            Instance selector - see `instance-selector` documentation on the DataTables site
       * @param {array}
       *            Button instance array that was attached to the DataTables settings object
      ...
      /**
       * Button selector - select one or more buttons from a selector input so some operation can be performed on them.
       * 
       * @param {array}
       *            Button instances array that the selector should operate on
       * @param {string|int|node|jQuery|array}
       *            Button selector - see `button-selector` documentation on the DataTables site
       * @return {array} Array of objects containing `inst` and `idx` properties of the selected buttons so you know which instance each button
       *         belongs to.
       * @static
       */
      ...
      /**
       * Buttons defaults. For full documentation, please refer to the docs/option directory or the DataTables site.
       * 
       * @type {Object}
       * @static
       */
      ...
      $.extend(_dtButtons, {
        collection : {
          ...
          action : function(e, dt, button, config) {
            ...
            config._collection.addClass(config.collectionLayout).css('display', 'none').appendTo('body').fadeIn(
                config.fade);
       ...
       * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * DataTables API
       * 
       * For complete documentation, please refer to the docs/api directory or the DataTables site
       */
      ...
      DataTable.Api.register('buttons.info()', function(title, message, time) {
        ...
        $('<div id="datatables_buttons_info" class="dt-button-info"/>').html(title).append(
            $('<div/>')[typeof message === 'string' ? 'html' : 'append'](message)).css('display', 'none').appendTo(
            'body').fadeIn();
      ...
      var _exportData = function(dt, inOpts) {
        ...
        var footer = dt.table().footer() ? dt.columns(config.columns).indexes().map(function(idx, i) {
